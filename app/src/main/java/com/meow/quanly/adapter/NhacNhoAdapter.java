...
import com.meow.quanly.NhacNhoSinhVienActivity;
...
public class NhacNhoAdapter extends RecyclerView.Adapter<NhacNhoAdapter.ViewHolder> implements Filterable {

    ArrayList<User> arr, arrFilter;

    Context context;
    public NhacNhoAdapter(Context context, ArrayList<User> arr) {
        this.arr = arr;
        this.arrFilter = arr;
        ...
        public void bind(final User item) {
            ...
            v.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    final DatabaseReference nhacnho = FirebaseDatabase.getInstance().getReference("notification").child(item.getUid());

                    final Dialog dialog = new Dialog(context);

                    dialog.setContentView(R.layout.dialog_nhacnho);

                    dialog.setTitle("Thông báo");
                    dialog.show();

                    final MaterialEditText text = dialog.findViewById(R.id.dialog_detail);
                    ...
                    btn.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            {
                                //nhacnho.push().setValue(new Notification(Common.uid, text.getText().toString(), System.currentTimeMillis()+"","", false));
                                nhacnho.push().setValue(new Notification(Common.uid, text.getText().toString(),item.getUid(), System.currentTimeMillis(), false));

                                //nhacnho.push().setValue(text.getText().toString());
                                Toast.makeText(context, "Đã gửi nhắc nhở đến sinh viên " + item.getUsername(), Toast.LENGTH_SHORT).show();
                                dialog.dismiss();
                            }
                        }
                    });
